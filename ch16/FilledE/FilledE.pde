...
void draw() {
  ...
  for(int i = 1; i < numPoints; i++) {
    vertex(points[i-1].screenX(), points[i-1].screenY());
    vertex(points[i].screenX(), points[i].screenY());
  }
  vertex(points[numPoints - 1].screenX(), points[numPoints - 1].screenY());
  vertex(points[0].screenX(), points[0].screenY());
  endShape();
}
